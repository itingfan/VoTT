    ...
    while sec < video_clip.end_time:
        ...
        for i_box, mean_box in enumerate(mean_boxes):
            ...
            region_list.append(Region(track_ids[i_box],
                                      default_type,
                                      track_id_to_tag[track_ids[i_box]],
                                      default_points,
                                      bbox))
        ...
        if cv2_visual:
            ...
            for i_box, box in enumerate(mean_boxes):
                (x, y, x2, y2) = [int(v) for v in box]
                # print((x, y), (x2, y2))
                cv2.rectangle(frame, (x, y), (x2, y2), (255, 255, 0), 2)
                font = cv2.FONT_HERSHEY_SIMPLEX
                cv2.putText(frame, str(track_id_to_tag[track_ids[i_box]]), (x, y), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
        ...
        if sec - video_clip.start_time < frame_rate:
            ...
            for region in init_regions:
                ...
                track_id_to_tag[region.id] = region.tags
                tracker = cv2.TrackerCSRT_create
...
if __name__ == '__main__':
    videoClip = VideoClip(r"C:\Users\tobai\Documents\data\mov01.mp4", 0.0000000, 10.000000)
    init_regions = []
    ...
    init_regions.append(Region(1,
                              ...
                              ["Tong"],
                              default_points,
                              BoundingBox(944, 385, 200, 700)))
    init_regions.append(Region(2,
                               default_type,
                               ["Kuan"],
                               default_points,
                               BoundingBox(1326, 337, 200, 700)))
